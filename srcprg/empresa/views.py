...
def nova_empresa(request):
    if request.method == "GET":
        ...
        # getlist porque vamos receber mais de um parametro
        tecnologias = request.POST.getlist('tecnologias')
        # vamos receber uma picture
        logo = request.FILES.get('logo')

        # strip retira os espaços em branco
        if (len(nome.strip()) == 0 or len(email.strip()) == 0 or len(cidade.strip()) == 0 or len(endereco.strip()) == 0 or len(nicho.strip()) == 0 or len(caracteristicas.strip()) == 0 or (not logo)): # noqa
            messages.add_message(request, constants.ERROR, 'Preencha todos os campos')
            return redirect('/home/nova_empresa')

        if logo.size > 100_000_000:  # logo não pode ser maior que 10MBits
...
def empresas(request):
    ...
    # se o usuario inserir tecnologias como filtro
    if technologias_filtrar:
        empresas = empresas.filter(tecnologias=technologias_filtrar)
    # se o usuario inserir nome como filtro
    if nome_filtrar:
